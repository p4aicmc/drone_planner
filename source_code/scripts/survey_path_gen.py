...
class PathPlanner(LifecycleNode):
    ...
    def on_activate(self, state: State) -> TransitionCallbackReturn:
        ...
        self.srv = self.create_service(GeneratePath, 'survey_path_gen/generate_path', self.generate_path_callback)
    ...
    def generate_path_callback(self, request, response):
        ...
        waypointsList = self.generate_path(self.drone_position, goal) or []
    ...
    def generate_path(self, start: tuple, goal: tuple ) -> list:
        ...
        for i in range(int(length/step)):
            ...
            s = v2(start[0], start[1])
            w = s + direction * (i * step) + direction2 * (sig * angle_step * i)
            waypoints.append(self.create_waypoint_message(w.to_list()))

            # w = v2(start[0], start[1]) + direction * (i * step) + direction2 * (sig * angle_step * i)
            # waypoints.append(self.create_waypoint_message(w.to_list()))

        for waypoint in waypoints:
            self.get_logger().info(f"Waypoint: {waypoint.pose.position.x}, {waypoint.pose.position.y}") # NOT_ESSENTIAL_PRINT

        self.get_logger().info(f"Generated ASD {len(waypoints)} waypoints") # NOT_ESSENTIAL_PRINT
